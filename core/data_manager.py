...
class DataManager:
    ...
    def _inicializar_bd(self):
        ...
        with self._get_conn() as conn:
            # 1. Crear tablas con la nueva estructura (incluyendo 'imagen')
            conn.executescript("""
            ...
            # 2. Migración segura para bases de datos existentes
            # Intenta agregar la columna 'imagen' a la tabla 'productos'.
            # Si la columna ya existe, SQLite lanzará un OperationalError que ignoraremos.
            try:
                conn.execute("ALTER TABLE productos ADD COLUMN imagen TEXT")
            except sqlite3.OperationalError:
                pass

            # 3. Insertar catálogo base solo si la tabla está vacía
            count = conn.execute("SELECT COUNT(*) FROM productos").fetchone()[0]
            if count == 0:
                catalogo_base = [
                    ("Mole Poblano",      45.0, 100, "mole_poblano.jpg"),
                    ("Enchiladas Verdes", 35.0, 100, "enchiladas_verdes.jpg"),
                    ("Chilaquiles Rojos", 30.0, 100, "chilaquiles_rojos.jpg"),
                    ("Pozole Rojo",       50.0, 100, None),
                    ("Chiles Rellenos",   40.0, 100, None),
                    ("Tlayuda Oaxaquena", 55.0, 100, None),
                ]
                conn.executemany(
                    "INSERT OR IGNORE INTO productos (nombre, precio, stock, imagen) VALUES (?, ?, ?, ?)",
                    catalogo_base
                )
    ...
    def get_inventario(self) -> dict:
        """Retorna el diccionario con todo el inventario de productos, incluyendo imágenes."""
        with self._get_conn() as conn:
            # Se agrega la columna 'imagen' a la consulta SQL
            rows = conn.execute(
                "SELECT nombre, precio, stock, imagen FROM productos ORDER BY id"
            ).fetchall()
            
        # Se empaqueta la imagen dentro del diccionario resultante
        return {
            r["nombre"]: {
                "precio": r["precio"], 
                "stock": r["stock"],
    ...
    def agregar_producto(self, nombre: str, precio: float, imagen: str = None, stock: int = 100) -> bool:
        """
        Agrega un nuevo producto al inventario.
        Soporta la inserción de la ruta de la imagen correspondiente.
        """
        try:
            with self._get_conn() as conn:
                conn.execute(
            ...
            return False
    def actualizar_imagen_producto(self, nombre: str, ruta_imagen: str):
        """Actualiza la ruta de la imagen de un platillo existente."""
        with self._get_conn() as conn:
            conn.execute(
    ...
    def deshacer_ultima_venta(self):
        ...
        # Reconstruir el dict con la misma estructura
    ...
    def cerrar_dia(self):
        ...
        También genera un archivo JSON de respaldo.
        """
        import json
