...
class DataManager:
    ...
    def _inicializar_bd(self):
        ...
        with self._get_conn() as conn:
            ...
            # Migración: Agregar columna topico a gastos si no existe
            cursor = conn.execute("PRAGMA table_info(gastos)")
            columnas = [col["name"] for col in cursor.fetchall()]
            if "topico" not in columnas:
                conn.execute("ALTER TABLE gastos ADD COLUMN topico TEXT DEFAULT 'General'")
    ...
    def registrar_gasto(self, concepto: str, monto: float, topico: str = "General"):
        """Registra un gasto del día."""
        fecha = datetime.now().strftime("%Y-%m-%d")
        with self._get_conn() as conn:
            conn.execute(
                "INSERT INTO gastos (fecha, concepto, monto, topico) VALUES (?, ?, ?, ?)",
                (fecha, concepto, monto, topico)
            )
    ...
    def get_gastos_hoy(self) -> list:
        ...
        with self._get_conn() as conn:
            rows = conn.execute(
                "SELECT id, concepto, monto, topico FROM gastos WHERE fecha = ? ORDER BY id",
                (fecha_hoy,)
            ).fetchall()
        return [{"id": r["id"], "concepto": r["concepto"], "monto": r["monto"], "topico": r["topico"]} for r in rows]

    def get_topicos_historicos(self) -> list:
        with self._get_conn() as conn:
            rows = conn.execute("SELECT DISTINCT topico FROM gastos WHERE topico IS NOT NULL AND topico != '' ORDER BY topico").fetchall()
        return [r["topico"] for r in rows]

    def eliminar_gasto(self, id_gasto: int):
        with self._get_conn() as conn:
            conn.execute("DELETE FROM gastos WHERE id = ?", (id_gasto,))

    def eliminar_gastos_por_topico_hoy(self, topico: str):
        fecha_hoy = datetime.now().strftime("%Y-%m-%d")
        with self._get_conn() as conn:
            conn.execute("DELETE FROM gastos WHERE fecha = ? AND topico = ?", (fecha_hoy, topico))
