...
class DataManager:
    ...
    def _inicializar_bd(self):
        with self._get_conn() as conn:
            conn.executescript("""
                ...
                -- Cuentas abiertas (mesas/clientes pendientes de cobro)
                -- carrito se guarda como JSON: {"Producto": cantidad, ...}
    ...
    # ─────────────────────────────────────────────
    # CUENTAS ABIERTAS (mesas / clientes)
    # ─────────────────────────────────────────────

    def get_cuentas_abiertas(self) -> list[dict]:
        """
        Retorna todas las cuentas abiertas con su carrito deserializado.
        Cada elemento: {"id": int, "nombre": str, "carrito": dict, "creada_en": str}
        """
        with self._get_conn() as conn:
            rows = conn.execute(
                "SELECT id, nombre, carrito, creada_en FROM cuentas_abiertas ORDER BY id"
            ).fetchall()
        return [
            {
                "id":        r["id"],
                "nombre":    r["nombre"],
                "carrito":   json.loads(r["carrito"]),
                "creada_en": r["creada_en"],
            }
            for r in rows
        ]

    def crear_cuenta_abierta(self, nombre: str) -> int:
        """
        Crea una nueva cuenta vacía en BD.
        Retorna el id generado para identificarla.
        """
        ahora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with self._get_conn() as conn:
            cursor = conn.execute(
                "INSERT INTO cuentas_abiertas (nombre, carrito, creada_en) VALUES (?, ?, ?)",
                (nombre, "{}", ahora)
            )
        return cursor.lastrowid

    def actualizar_carrito_cuenta(self, cuenta_id: int, carrito: dict):
        """Persiste el estado actual del carrito de una cuenta."""
        with self._get_conn() as conn:
            conn.execute(
                "UPDATE cuentas_abiertas SET carrito = ? WHERE id = ?",
                (json.dumps(carrito, ensure_ascii=False), cuenta_id)
            )

    def eliminar_cuenta_abierta(self, cuenta_id: int):
        """Elimina la cuenta de la BD (al cobrar o cerrar sin cobrar)."""
        with self._get_conn() as conn:
            conn.execute(
                "DELETE FROM cuentas_abiertas WHERE id = ?", (cuenta_id,)
            )
