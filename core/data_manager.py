...
class DataManager:
    ...
    def get_historial_hoy(self) -> list:
        """Retorna lista de ventas del día actual con id, hora y total."""
    ...
    def editar_venta(self, venta_id: int, nuevo_carrito: dict) -> bool:
        """
        Reemplaza los productos de una venta existente y recalcula su total.
        Restaura el stock de los productos anteriores y descuenta el nuevo carrito.
        Retorna False si el carrito está vacío o la venta no existe.
        """
        ...
        with self._get_conn() as conn:
            venta = conn.execute(
                "SELECT id FROM ventas WHERE id = ?", (venta_id,)
            ).fetchone()
            if not venta:
                return False

            # 1. Restaurar stock de los productos anteriores
            detalles_anteriores = conn.execute(
                "SELECT producto, cantidad FROM venta_detalle WHERE venta_id = ?",
                (venta_id,)
            ).fetchall()
            ...
            # 3. Insertar nuevo detalle, descontar stock y calcular nuevo total
            inventario = {
                r["nombre"]: r["precio"]
                for r in conn.execute("SELECT nombre, precio FROM productos").fetchall()
            }
            nuevo_total = 0.0
            for prod, cant in nuevo_carrito.items():
                conn.execute(
                    "INSERT INTO venta_detalle (venta_id, producto, cantidad) VALUES (?, ?, ?)",
                    (venta_id, prod, cant)
                )
                ...
                nuevo_total += cant * inventario.get(prod, 0)

            # 4. Actualizar total de la venta
            conn.execute(
                "UPDATE ventas SET total = ? WHERE id = ?",
                (round(nuevo_total, 2), venta_id)
            )
    ...
    def get_gastos_hoy(self) -> list:
        """Retorna lista de gastos del día actual con id, concepto y monto."""
        fecha_hoy = datetime.now().strftime("%Y-%m-%d")
        with self._get_conn() as conn:
            rows = conn.execute(
                "SELECT id, concepto, monto FROM gastos WHERE fecha = ? ORDER BY id",
                (fecha_hoy,)
            ).fetchall()
        return [{"id": r["id"], "concepto": r["concepto"], "monto": r["monto"]} for r in rows]

    def editar_gasto(self, gasto_id: int, concepto: str, monto: float) -> bool:
        """Actualiza el concepto y monto de un gasto existente."""
        with self._get_conn() as conn:
            cursor = conn.execute(
                "UPDATE gastos SET concepto = ?, monto = ? WHERE id = ?",
