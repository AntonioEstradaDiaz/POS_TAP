...
def migrar_inventario(conn: sqlite3.Connection):
    ...
    for nombre, data in inventario.items():
        try:
            # Se agrega 'imagen' con valor None ya que los JSON no lo tenían
            conn.execute(
                "INSERT INTO productos (nombre, precio, stock, imagen) VALUES (?, ?, ?, ?)",
                (nombre, data.get("precio", 0), data.get("stock", 100), None)
            )
            insertados += 1
...
def crear_tablas(conn: sqlite3.Connection):
    ...
    Se actualizó para incluir la columna 'imagen'.
    """
    conn.executescript("""
...
def seed_data_si_esta_vacio(conn: sqlite3.Connection):
    ...
    Actualizado para incluir las rutas de las imágenes.
    """
