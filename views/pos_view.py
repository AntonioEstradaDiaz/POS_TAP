...
class POSView(ft.Container):
    ...
    def _cobrar(self, e):
        ...
        if total > 0:
            ...
            ticket_carrito = self.carrito.copy()
            self.carrito.clear()
            ...
            self.main_page.overlay.append(snack)
            self._mostrar_ticket(ticket_carrito,total)
            snack.open = True
            self.main_page.update()
    ...
    def _generar_ticket_pdf(self,carrito,total):
        ahora = datetime.now()
        ...
        c.drawString(180, y, "TICKET DE COMPRA")

        y -= 40

        c.setFont("Helvetica", 12)
        c.drawString(50, y, "Lugar: Antojitos POS")

        y -= 20
        c.drawString(50, y, f"Fecha: {ahora.strftime('%Y-%m-%d')}")

        y -= 20
        c.drawString(50, y, f"Hora: {ahora.strftime('%H:%M:%S')}")
        ...
        c.drawString(320, y, f"TOTAL: ${total:.2f}")

        y -= 40

        c.setFont("Helvetica", 11)
        c.drawString(180, y, "Gracias por su compra")
        return ruta_pdf
    
    def _mostrar_ticket(self, carrito, total):
        ahora = datetime.now()

        lista_productos = ft.Column(
            controls=[
                ft.Row([
                    ft.Text(f"{cant}x", width=40, color="white"),
                    ft.Text(prod, expand=True, color="white"),
                    ft.Text(
                        f"${self.inventario[prod]['precio'] * cant:.2f}",
                        color="#38bdf8"
                    )
                ])
                for prod, cant in carrito.items()
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Container(
                width=420,
                content=ft.Column([
                    ft.Text("Antojitos POS", size=20, weight="bold", color="white"),
                    ft.Text("Lugar: Puebla, México", color="#94a3b8"),
                    ft.Text(
            ...
            actions=[
                ft.ElevatedButton(
                    ...
                    on_click=lambda e: webbrowser.open(
                        self._generar_ticket_pdf(carrito, total)
                    )
                ),
                ft.ElevatedButton(
                    "Descargar PDF",
                    icon=Icons.DOWNLOAD,
                    on_click=lambda e: self._descargar_ticket(carrito, total)
                ),
                ft.TextButton(
                    "Cerrar",
    ...
    def _descargar_ticket(self, carrito, total):
        ruta = self._generar_ticket_pdf(carrito, total)

        snack = ft.SnackBar(
            ft.Text(f"PDF guardado en:\n{ruta}"),
            bgcolor="#166534"
        )
