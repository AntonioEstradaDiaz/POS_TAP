...
from core.venta_rapida import aplicar_boton
...
class VentasView(ft.Container):
    ...
    def __init__(self, page, data_manager):
        ...
        self.botones_cantidad = []
        for i in range(1, 10):
            btn = ft.ElevatedButton(
                str(i),
                on_click=lambda e, n=i: self.seleccionar_cantidad(n),
                bgcolor="#38bdf8" if i == 1 else "#1e293b",
                color="#0f172a" if i == 1 else "white",
                style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=20)),
                width=44, height=44,
            )
            btn.data = i  # guardamos el número en .data para comparar después
            self.botones_cantidad.append(btn)
        self.numeros_row = ft.Row(
            controls=self.botones_cantidad,
            alignment="center",
            spacing=5
        )
    ...
    def seleccionar_cantidad(self, numero: int):
        """Actualiza la cantidad seleccionada y resalta el botón activo."""
        self.cantidad_seleccionada = numero
        for btn in self.botones_cantidad:
            activo = (btn.data == numero)
            btn.bgcolor = "#38bdf8" if activo else "#1e293b"
            btn.color   = "#0f172a" if activo else "white"
        self.numeros_row.update()
    ...
    def _add_to_cart(self, prod: str, e):
        # 🔥 BOTÓN RÁPIDO (usa tu lógica nueva)
        boton = {
            "nombre": prod,
            "productos": [prod]
        }
        ...
        ticket_temp = []
        ticket_temp = aplicar_boton(boton, ticket_temp)

        # 👉 Agregamos al carrito usando la cantidad seleccionada
        for producto in ticket_temp:
            self.carrito[producto] = self.carrito.get(producto, 0) + self.cantidad_seleccionada

        # Resetear cantidad a 1 después de agregar
        self.seleccionar_cantidad(1)
